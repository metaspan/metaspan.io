...
const CACHE_URL = cfg.url || 'https://polkadot.w3f.community/nominators'
...
module.exports = {
  ...
  async fn (inputs, exits, env) {
    ...
    if (moment().diff(updatedAt, 'seconds') > CACHE_VALIDITY && !loading) {
      sails.log.debug('polkadot/nominator.js: updating cache')
      loading = true
      try {
      ...
      sails.log.debug('polkadot/nominator.js: serving from cache - ' + updatedAt.toString()) //.format(DATETIME_FORMAT)
    }
