<template>
  <v-container :loading="loading" fluid>
    ...
    <v-toolbar class="d-none d-sm-block" dense flat elevation="0">
      ...
      <v-text-field v-model="xfilter.total" label="Score" class="mx-4"></v-text-field>
    ...
    <v-dialog v-model="showFilterDialog">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-switch v-model="xfilter.sortAsc" value="1" label="Ascending"></v-switch>
        </v-card-text>
      </v-card>
      <v-card>
...
export default defineComponent({
  ...
  setup () {
    ...
    const loading = computed(() => polkadotStore.loading)
    const updatedAt = computed(() => polkadotStore.updatedAt)
    const nominators = computed(() => polkadotStore.nominators)
    ...
    const xfilter = ref<ICandidateListFilter>({
        rank: undefined,
        total: undefined,
        valid: false,
        active: false,
    ...
    watch(() => xfilter.value, newVal => {
      ...
      candidateStore.handleFilter(newVal)
    })
    ...
    onBeforeMount(() => {
      ...
      search.value = candidateStore.search
    })
